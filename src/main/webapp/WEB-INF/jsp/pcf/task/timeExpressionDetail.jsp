...
<%@include file="/WEB-INF/jsp/common/import-corn.jsp"%>
<style type="text/css">
	.line {
		height: 25px;
		line-height: 25px;
...
<body>
<div class="easyui-layout" style="width:860px;height:500px; border: 1px rgb(202, 196, 196) solid;border-radius: 5px;">
...
<div data-options="region:'south',border:false" style="height:180px">
	<fieldset style="border-radius: 3px; height: 110px;">
		<legend>表达式</legend>
		<table style="height: 100px;">
			<tbody>
				...
				<tr>
					<td>Cron 表达式:</td>
					<td colspan="6"><input type="text" name="cron" disabled='disabled'
						style="width: 100%;" value="${cron}" id="cron" /></td>
					<td><input type="hidden" value="反解析到UI " id="btnFan"
						onclick="btnFan()" /></td>
				</tr>
			</tbody>
		</table>
...
<div style="text-align: center; margin-top: 5px;">
		<script type="text/javascript">
			...
			/*把生成的时间表达式,传递给父类*/
			function doOk(){
				parent.setCron($("input[name='cron']").val());
			}
			...
			function initObj(strVal, strid) {
				...
				if (strVal == "*") {
					...
					$("#" + strid + "Start_0").numberspinner(
							'setValue', ary[0]);
					$("#" + strid + "End_0").numberspinner('setValue',
							ary[1]);
				} else if (strVal.split('/').length > 1) {
					ary = strVal.split('/');
					...
					$("#" + strid + "Start_1").numberspinner(
							'setValue', ary[0]);
					$("#" + strid + "End_1").numberspinner('setValue',
							ary[1]);
				} else {
