...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        jQuery(grid_selector).jqGrid(jqGridOption({
            url:'${ctx}/buddha/server/scripture/scriptureByConsList',
            datatype: "json",
            height: 350,
            shrinkToFit: false,
            colNames: [' ','标题','类型','翻译者','开示者','创建帐户','','状态','排序键','创建日期','更新日期'], 
            colModel: [
                {name: 'id', index: 'id', width: 60, sorttype: "int",hidden:true},
                {name: 'title', index: 'title', width: 300, sorttype: "string",hidden:false},
                {name: 'ptTitle', index: 'ptTitle', width: 100, sorttype: "string",hidden:false},
                {name: 'translator', index: 'translator', width: 120, edittype: "string", hidden:false},
                {name: 'disclosure', index: 'disclosure', width: 120, edittype: "string", hidden:false},
        ...
        gridTools.jqGrid("navButtonAdd", "#grid-pager-school", {
            id: "addCase",
            caption: "<button id='addBtn' class='btn btn-xs btn-primary'>添加经文</button>",
            buttonicon: "none"
        });
        gridTools.jqGrid("navButtonAdd", "#grid-pager-school", {
            id: "updateCase",
            caption: "<button id='updateBtn' class='btn btn-xs btn-primary'>修改经文</button>",
            buttonicon: "none"
        });
        gridTools.jqGrid("navButtonAdd", "#grid-pager-school", {
            id: "deleteCase",
            caption: "<button id='deleteBtn' class='btn btn-xs btn-primary'>删除经文</button>",
            buttonicon: "none"
        });
        gridTools.jqGrid("navButtonAdd", "#grid-pager-school", {
            id: "stopCase",
            caption: "<button id='stopBtn' class='btn btn-xs btn-primary'>停用经文</button>",
            buttonicon: "none"
        });
        gridTools.jqGrid("navButtonAdd", "#grid-pager-school", {
            id: "useCase",
            caption: "<button id='useBtn' class='btn btn-xs btn-primary'>启用经文</button>",
            buttonicon: "none"
        });
        gridTools.jqGrid("navButtonAdd", "#grid-pager-school", {
            id: "viewCase",
            caption: "<button id='viewBtn' class='btn btn-xs btn-primary'>查看经文信息</button>",
            buttonicon: "none"
        });
        // 增加
        $("#addBtn").click(function () {
            document.location.href = "${ctx}/buddha/server/scripture/scriptureDetail?param=add";
        });
        // 修改
        $("#updateBtn").click(function () {
            ...
            if (selr) {
                ...
                document.location.href = "${ctx}/buddha/server/scripture/scriptureDetail?scriptureId=" +row.id+"&param=edit";
            } else {
                addGritter("error", "请选择您要修改的经文");
            }
        });
        ...
        // 删除
      	$("#deleteBtn").click(function () {
            ...
            if (selrArray.length>0) {
                bootbox.confirm("您确认要删除选中的经文吗?删除以后将无法恢复", function(result) {
                    if(result) {
                        ...
                        doPost("${ctx}/buddha/server/scripture/deleteScripture?scriptureIdList=" +scriptureIdList, function (data) {
                            if (data.success) {
            ...
            if (selrArray.length>0) {
                bootbox.confirm("您确认要停用选中的经文吗?", function(result) {
                    if(result) {
                        ...
                        doPost("${ctx}/buddha/server/scripture/updateScriptureStatus?scriptureIdList=" +scriptureIdList+"&status=0", function (data) {
                            if (data.success) {
                                addGritter("success", data.message);
                                jQuery("#grid-table-school").trigger("reloadGrid");
                            } else {
            ...
            if (selrArray.length>0) {
                bootbox.confirm("您确认要启用选中的经文吗?", function(result) {
                    if(result) {
                        ...
                        doPost("${ctx}/buddha/server/scripture/updateScriptureStatus?scriptureIdList=" +scriptureIdList+"&status=1", function (data) {
                            if (data.success) {
                                addGritter("success", data.message);
                                jQuery("#grid-table-school").trigger("reloadGrid");
                            } else {
