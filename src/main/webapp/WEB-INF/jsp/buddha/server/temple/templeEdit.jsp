...
<div class="row">
    <div class="col-sm-12">
        ...
        <form class="form-horizontal" role="form" id="templeForm">
            <input id="id" name="id" value="${temple.id}" type="hidden">
            <input id="pId" name="pId" value="${temple.provinceId}" type="hidden">
            <input id="cId" name="cId" value="${temple.cityId}" type="hidden">
            <input id="ctId" name="ctId" value="${temple.countyId}" type="hidden">
            <input id="editStatus" name="editStatus" value="${temple.status}" type="hidden">
            <input id="editIsRz" name="editIsRz" value="${temple.isRz}" type="hidden">
            <input id="editIsTj" name="editIsTj" value="${temple.isTj}" type="hidden">
            <div style="display: none" id="divBanner">${temple.banner}</div>
            <div style="display: none" id="divRemark">${temple.remark}</div>
            <div class="space-1"></div>
            <!-- #section:elements.form -->
            <div class="form-group">
                <label class="col-sm-1 control-label no-padding-right" for="name">名称<span style="color: red">*</span></label>
                <div class="col-sm-9">
                    <input type="text" id="name" name="name" value="${temple.name}" placeholder="寺院名称" class="col-xs-10 col-sm-5" onBlur="validateNameIsExist()"/>
                </div>
            </div>
			...
			<div id="divCover" class="form-group">
                ...
                <div class="col-sm-9">
                    <textarea id="banner" name="banner">${temple.banner}</textarea>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <input type="text" id="buddhistName" name="buddhistName" value="${temple.buddhistName}" placeholder="住持" class="col-xs-10 col-sm-5"/>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <input type="text" id="address" name="address" value="${temple.address}" placeholder="寺院地址" class="col-xs-10 col-sm-5"/>
                    &nbsp;经度:<label id="lngLab" > ${temple.lng}</label><input type="text" hidden="hidden" name="lng" id="lng"  value="${temple.lng}">
                    </br>
                    &nbsp;纬度:<label id="latLab" > ${temple.lat}</label><input type="text" hidden="hidden"  name = "lat"  id="lat"  value="${temple.lat}">
                    <div id = 'message'></div>
                </div>
            </div>
            <div class="form-group">
            ...
            <div id="divCover" class="form-group">
                ...
                <div class="col-sm-9">
                    <select id="provinceId" name="provinceId" onchange="getCity()" value="${temple.provinceId}" style="width:150px">
                            <option value=''>请选择</option>
                        </select>
                        <select id="cityId" name="cityId" onchange="getCounty()" value="${temple.cityId}" style="width:150px">
                            <option value=''>请选择</option>
                        </select>
                        <select id="countyId" name="countyId" value="${temple.countyId}" style="width:150px">
                            <option value=''>请选择</option>
                        </select>
                </div>
            </div>
			...
			<div class="form-group">
                ...
                <div class="col-sm-9">
                    <input type="text" id="sort" name="sort" value="${temple.sort}" placeholder="排序键" class="col-xs-10 col-sm-5"/>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <textarea id="remark" name="remark">${temple.remark}</textarea>
                    <script type="text/javascript"> 
...
<script type="text/javascript">
	$(function(){
		...
		initProvinces();
		$("select[name='provinceId']").find("option[value='" + $("#pId").val() + "']").attr("selected", true);
	    getCity();
	    $("select[name='cityId']").find("option[value='" + $("#cId").val() + "']").attr("selected", true);
	    getCounty();
	    $("select[name='countyId']").find("option[value='" + $("#ctId").val() + "']").attr("selected", true);
        // 提交操作
        $(".subm").on("click", function () {
        	if (!$('#templeForm').valid()) {
                return false;
            }
            doPost("${ctx}/buddha/server/temple/updateTemple", $("#templeForm").serialize(), function (data) {
            	if (data.success) {
    ...
    function clearUeditor() {
	    ...
	    UE.getEditor('remark').setContent($("#divRemark").html());
	}
