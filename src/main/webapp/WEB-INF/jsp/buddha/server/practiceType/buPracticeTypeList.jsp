    ...
    <div class="col-sm-12">
        <form class="form-horizontal well well-sm queryForm">
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <label class="col-sm-3 control-label no-padding-right" for="bType">状态</label>
                        <div class="col-sm-9">
                                <select class="form-control" id="bType" name="search.a.bType_eq">
                                    <option value="">请选择</option>
                                    <option value="2">藏经阁</option>
...
<script type="text/javascript">
    jQuery(function ($) {
        ...
        jQuery(grid_selector).jqGrid(jqGridOption({
            url:'${ctx}/buddha/server/practiceType/buPracticeTypeByConsList',
            datatype: "json",
            height: 350,
            shrinkToFit: false,
            colNames: [' ','名称','','类型','','状态','创建人','创建日期','更新人','更新日期'],
            colModel: [
                {name: 'id', index: 'id', width: 60, sorttype: "int",hidden:true},
                {name: 'title', index: 'title', width: 120, sorttype: "string",hidden:false},
                {name: 'bType', index: 'bType', width: 120, sorttype: "string",hidden:true},
                {name: 'bTypeStr', index: 'bTypeStr', width: 120, sorttype: "string",hidden:false},
...
//        gridTools.jqGrid("navButtonAdd", "#grid-pager-school", {
//           id: "deleteCase",
//            caption: "<button id='deleteBtn' class='btn btn-xs btn-primary'>删除修行类型</button>",
//            buttonicon: "none"
//        });
        gridTools.jqGrid("navButtonAdd", "#grid-pager-school", {
            id: "stopCase",
...
//        gridTools.jqGrid("navButtonAdd", "#grid-pager-school", {
//            id: "viewCase",
//            caption: "<button id='viewBtn' class='btn btn-xs btn-primary'>查看修行类型信息</button>",
//            buttonicon: "none"
//        });
        // 增加
        $("#addBtn").click(function () {
            document.location.href = "${ctx}/buddha/server/practiceType/buPracticeTypeDetail?param=add";
        });
        // 修改
        $("#updateBtn").click(function () {
            ...
            if (selr) {
                ...
                document.location.href = "${ctx}/buddha/server/practiceType/buPracticeTypeDetail?buPracticeTypeId=" +row.id+"&param=edit";
            } else {
                addGritter("error", "请选择您要修改的修行类型");
            }
        });
        ...
        $("#stopBtn").click(function () {
            ...
            if (selrArray.length>0) {
                bootbox.confirm("您确认要停用选中的修行类型吗?", function(result) {
                    if(result) {
                        ...
                        doPost("${ctx}/buddha/server/practiceType/updatePracticeTypeStatus?buPracticeTypeIdList=" +serveIdList+"&status=0", function (data) {
                            if (data.success) {
        ...
        $("#useBtn").click(function () {
            ...
            // 必须要有一个删除的对象
            if (selrArray.length>0) {
                bootbox.confirm("您确认要启用选中的修行类型吗?", function(result) {
                    if(result) {
                        var serveIdList= new Array();
                        for(var i=0; i< selrArray.length; i++){
                            var ret = jQuery(grid_selector).jqGrid('getRowData', selrArray[i]);
                        ...
                        doPost("${ctx}/buddha/server/practiceType/updatePracticeTypeStatus?buPracticeTypeIdList=" +serveIdList+"&status=1", function (data) {
                            if (data.success) {
                                addGritter("success", data.message);
                                jQuery("#grid-table-school").trigger("reloadGrid");
                            } else {
        ...
        $("#viewBtn").click(function () {
            ...
            if (selr) {
                var row = jQuery(grid_selector).jqGrid('getRowData', selr);
                document.location.href = "${ctx}/buddha/server/practiceType/buPracticeTypeView?practiceTypeId=" +row.id;
            } else {
                addGritter("error", "请选择您要查看的修行类型信息");
            }
        });
