...
<body class="no-skin" style="background-color: #fff">
			...
			<form class="form-horizontal well well-sm queryForm">
					...
					<div class="col-md-3">
						<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right"
								for="position">标题</label>
							<div class="col-sm-9">
								<input type="text" class="form-control" placeholder="请输入标题"
									id="title" name="search.a.title_like" />
							</div>
						</div>
					</div>
					<div class="col-md-3">
						<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right"
								for="ancestralHome">翻译者</label>
							<div class="col-sm-9">
								<input type="text" class="form-control" placeholder="请输入翻译者"
									id="translator" name="search.a.translator_like" />
							</div>
						</div>
					</div>
				</div>
				<div class="row">
					<div class="col-md-3">
						<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right"
								for="degree">开示者</label>
							<div class="col-sm-9">
								<input type="text" class="form-control" placeholder="请输入开示者"
									id="degree" name="search.a.disclosure_like" />
							</div>
						</div>
					</div>
					<div class="col-md-3">
    ...
    jQuery(function ($) {
        ...
        jQuery(grid_selector).jqGrid(jqGridOption({
            url:'${ctx}/buddha/server/possessions/getListByConsList',
            datatype: "json",
            height: 350,
            shrinkToFit: false,
            colNames: ['','名称','市场价格','优惠价格','排序键','状态','创建帐户','创建日期','更新帐户','更新日期'], 
            colModel: [
                {name: 'id', index: 'id', width: 60, sorttype: "int",hidden:true},
                {name: 'name', index: 'name', width: 300, sorttype: "string",hidden:false},
                {name: 'price', index: 'price', width: 100, sorttype: "string",hidden:false},
                {name: 'discountPrice', index: 'discountPrice', width: 100, sorttype: "string",hidden:false},
                {name: 'sort', index: 'sort', width: 120, edittype: "int", hidden:false},
        ...
        $("#addBtn").click(function () {
            document.location.href = "${ctx}/buddha/server/possessions/detail?opt=add";
        });
        // 修改
        $("#updateBtn").click(function () {
            ...
            if (selr) {
                ...
                document.location.href = "${ctx}/buddha/server/possessions/detail?id=" +row.id+"&opt=edit";
            } else {
            ...
            if (selrArray.length>0) {
                bootbox.confirm("您确认要停用选中的项目吗?", function(result) {
                    if(result) {
                        ...
                        doPost("${ctx}/buddha/server/possessions/updateStatus?idList=" +ids+"&status=0", function (data) {
                            if (data.success) {
            ...
            if (selrArray.length>0) {
                bootbox.confirm("您确认要启用选中的项目吗?", function(result) {
                    if(result) {
                        ...
                        doPost("${ctx}/buddha/server/possessions/updateStatus?idList=" +ids+"&status=1", function (data) {
                            if (data.success) {
                                addGritter("success", data.message);
                                jQuery("#grid-table-school").trigger("reloadGrid");
                            } else {
        ...
        $("#viewBtn").click(function () {
            ...
            if (selr) {
                var row = jQuery(grid_selector).jqGrid('getRowData', selr);
                document.location.href = "${ctx}/buddha/server/possessions/view?id=" +row.id;
            } else {
